...
void SearchContent(struct listNode *list)
{
	...
	while((scanf("%d", &choose)!=1) || (choose<0||choose>1))
	{
...
void wordFormation(struct listNode *list)
{
	...
	fflush(stdin);
	//提示用户输入数字对应英文单词的词性
	printf("请输入数字对应英文单词的词性(0-3):");

	//用户输入数字不在（0-3）间时提示用户输入错误，请重新输入
	while((scanf("%d", &choose)!=1) || (choose<N||choose>ADV))
	{
...
void wordFormation1(struct listNode *list,int choose)
{
	char *temp = (char *)malloc(WORDNUM+1);
	//创建字符指针把上述用户选择符号化
	char *formation=temp;
...
int main(int argc, char *argv[])
{
	...
	while (function != EXIT)
	{
		...
		while (scanf("%d", &function)!=1 || (function<EXIT || function>READ))
		{
